...
TEST_CASE("WebAPI add/get/remove functions") {
  ...
  bool added = api.addPlantToInventory("RosePlant", "flowering", "blooming",
                                       "spring", 9.99);
  REQUIRE(added == true);
  CHECK(inv.getSize() == 1);
  ...
  api.addPlantToInventory("CactusPlant", "succulent", "dormant", "winter",
                          4.50);
  CHECK(inv.getSize() == 2);
...
TEST_CASE("WebAPI json format works correctly") {
  ...
  bool works = (!json.empty() &&
                (json.back() == '}' || json.find("]}") != std::string::npos ||
