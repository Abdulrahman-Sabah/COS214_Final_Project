...
TEST_CASE("Rose Product") {
    ...
    CHECK(check);
    CHECK(r.description() == "Rose (Rosa): a fragrant and colorful flowering plant symbolizing love and beauty.");
    CHECK(r.price() > 0.0);
}
...
TEST_CASE("Lavender") {
    ...
    CHECK(check);
    CHECK(l.description() == "Lavender (Lavandula): a fragrant purple herb often used for decoration and scent.");
    CHECK(l.price() > 0.0);
}
...
TEST_CASE("Cactus") {
    ...
    CHECK(check);
    CHECK(c.description() == "Cactus (Cactaceae): a hardy, water-efficient plant adapted to dry climates.");
    CHECK(c.price() > 0.0);
}
