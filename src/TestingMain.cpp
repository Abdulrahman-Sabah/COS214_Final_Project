...
bool assertState(const std::string &testName, bool condition) {
  if (condition) {
    ...
    std::cout << ANSI_RED << "FAIL: " << testName
              << " , Condition failed, it was false\n"
              << ANSI_RESET;
    return false;
  }
}
...
int main() {
  ...
  std::cout << "Cloned Description: " << clonedLavender->description()
            << std::endl;
  std::cout << "Cloned Care Type: " << clonedLavender->getCareType()
            << std::endl;
  ...
  cout << "Name changed to: " << rosethree->getName()
       << " (no notification expected)" << endl;

  cout << "\n5. Detaching observer and making changes:" << endl;
  rosethree->detach(scheduler);
  rosethree->setStateText("Mature");
  cout << "State changed to Mature (no notification expected - observer "
          "detached)"
       << endl;
  ...
  cout << "Clone successful: "
       << (giftTulip->price() == clonedGiftTulip->price() ? "YES" : "NO")
       << endl;
  cout << endl;
  ...
  inventory->addPlant(
      new SimplePlant("Red Rose", "Moderate", "Budding", "Spring", 45.99));
  inventory->addPlant(
      new SimplePlant("Aloe Vera", "Low", "Mature", "Summer", 25.50));
  inventory->addPlant(
      new SimplePlant("Barrel Cactus", "Low", "Young", "Summer", 35.00));
  inventory->addPlant(
      new SimplePlant("Lavender", "Moderate", "Flowering", "Spring", 28.75));
  inventory->addPlant(
      new SimplePlant("Baobab Tree", "High", "Seedling", "Year-round", 150.00));
  inventory->addPlant(
      new SimplePlant("Jade Plant", "Low", "Mature", "Year-round", 32.00));
  inventory->addPlant(
      new SimplePlant("Peace Lily", "Moderate", "Flowering", "Spring", 55.00));
  inventory->addPlant(
      new SimplePlant("Snake Plant", "Low", "Mature", "Year-round", 40.00));
  ...
  std::cout << "Iterator A counted: " << posA
            << ", Iterator B counted: " << posB << "\n\n";
  delete itA;
  delete itB;
  ...
  std::cout << "   Command Type: " << deliveryCmd->getType()
            << ", Data: " << deliveryCmd->getData() << std::endl;
  deliveryStaff->handleRequest(deliveryCmd);

  std::cout << "\n2. Testing MoveToSales command:" << std::endl;
  std::cout << "   Command Type: " << salesCmd->getType()
            << ", Data: " << salesCmd->getData() << std::endl;
  deliveryStaff->handleRequest(salesCmd);

  std::cout << "\n3. Testing AddWater command:" << std::endl;
  std::cout << "   Command Type: " << waterCmd->getType()
            << ", Data: " << waterCmd->getData() << std::endl;
  deliveryStaff->handleRequest(waterCmd);

  std::cout << "\n4. Testing ApplyFertilizer command:" << std::endl;
  std::cout << "   Command Type: " << fertilizerCmd->getType()
            << ", Data: " << fertilizerCmd->getData() << std::endl;
  deliveryStaff->handleRequest(fertilizerCmd);

  std::cout << "\n--- Expected Results ---" << std::endl;
  std::cout << "ScheduleDelivery -> Handled by DeliveryStaff" << std::endl;
  std::cout << "MoveToSales -> Passed through DeliveryStaff -> Handled by "
               "GreenhouseManager"
            << std::endl;
  std::cout << "AddWater -> Passed through DeliveryStaff -> GreenhouseManager "
               "-> Handled by Landscaper"
            << std::endl;
  std::cout << "ApplyFertilizer -> Passed through DeliveryStaff -> "
               "GreenhouseManager -> Handled by Landscaper"
            << std::endl;
