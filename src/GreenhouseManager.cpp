...
void GreenhouseManager::handleRequest(Commands* r, Plant* plant) {
  ...
  if (r && (r->name() == "LavenderStrategy" || r->name() == "RoseStrategy" ||
        ...
        std::cout << "Greenhouse Manager: Executing "
                  << r->name() << " on "
                  << plant->getName() << ".\n";
    ...
    // Pass to next handler in chain using protected method
       std::cout << "Greenhouse Manager: This request is not my responsibility. My  responsibility is the Strategy(Rose, Cactus,Lavender)"
                     "Calling Landscaper...\n";
