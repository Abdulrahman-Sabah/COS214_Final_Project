...
public:
    PlantNurseryServer() {
        ...
        std::cout << "🌿 Server loaded " << inventory.getSize() << " plants from file\n";
    }
    ...
    bool start(int port = 8081) {
        ...
        std::cout << "🌿 Plant Nursery Server running on http://localhost:" << port << std::endl;
...
std::string handleHTTPRequest(const std::string& request) {
        ...
        if (request.find("POST /api/care/") != std::string::npos) {
            return "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n{\"status\":\"success\",\"message\":\"Plant cared for\"}";
        }
    ...
    std::string serveFile(const std::string& filename, const std::string& contentType) {
        ...
        std::cout << "📄 Served: " << filename << " (" << buffer.str().length() << " bytes)" << std::endl;
        return response;
    }
};
